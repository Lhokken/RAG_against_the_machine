...
from langchain_text_splitters import HTMLHeaderTextSplitter # .md in html
		...
		try:
			for file in file_list:
				if file.lower().endswith(".txt"):
					...
					with open(file) as source:
						if re.search(html_pattern, source.read(), re.IGNORECASE):
							temp = html_chunker.split_text(source.read())
						else:
							temp = md_chunker.split_text(source.read())
						self.chunk_list.extend(txt_chunker.split_documents(temp))
